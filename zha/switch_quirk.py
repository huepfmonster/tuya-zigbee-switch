...
class CustomOnOffConfigurationCluster(CustomCluster, OnOffConfiguration):
    ...
    class AttributeDefs(OnOffConfiguration.AttributeDefs):
        ...
        long_press_heartbeat_interval = ZCLAttributeDef(
            id=0xff06,
            type=t.uint16_t,
            access="rw",
            is_manufacturer_specific=False,
...
for config in CONFIGS:
    ...
    for endpoint_id in range(1, switch_cnt + 1):
        builder = (
            ...
            .number(
                CustomOnOffConfigurationCluster.AttributeDefs.long_press_heartbeat_interval.name,
                CustomOnOffConfigurationCluster.cluster_id,
                translation_key="long_press_heartbeat_interval_"+str(endpoint_id),
                fallback_name="Long press heartbeat interval "+str(endpoint_id),
                min_value=0,
                max_value=10000,
                step=1,
                endpoint_id=endpoint_id,
            ...
            .sensor(
                ...
                attribute_converter = lambda x: {0: "released", 1: "press", 2: "long_press", 3: "position_on", 4: "position_off", 5: "long_press_alt"}[int(x)]
            )
        )
